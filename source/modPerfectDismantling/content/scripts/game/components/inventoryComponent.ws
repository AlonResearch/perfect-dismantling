...
import class CInventoryComponent extends CComponent
{
	...
	public function PerfectDismantling_GetDismantlingParts( id : SItemUniqueId ) : array<SItemParts>
	{
		var parts : array<SItemParts>;
		var vanillaParts : array<SItemParts>;
		var upgrades : array<name>;
		var i : int;
		
		parts = PerfectDismantling_GetCraftingParts( id );
		if(parts.Size() <= 0)
		{
			vanillaParts = GetItemRecyclingParts( id );
			for(i=0; i<vanillaParts.Size(); i+=1)
				PerfectDismantling_AddOrStackPart(parts, vanillaParts[i].itemName, vanillaParts[i].quantity);
		}
		
		GetItemEnhancementItems( id, upgrades );
		for(i=0; i<upgrades.Size(); i+=1)
			PerfectDismantling_AddOrStackPart(parts, upgrades[i], 1);
		
		return parts;
	}
	
	private final function PerfectDismantling_GetCraftingParts( id : SItemUniqueId ) : array<SItemParts>
	{
		var dm : CDefinitionsManagerAccessor;
		var main, ingredients : SCustomNode;
		var itemName, tmpName : name;
		var tmpInt : int;
		var craftedCount : int;
		var parts : array<SItemParts>;
		var i, k : int;
		...
		{
			if(dm.GetCustomNodeAttributeValueName(main.subNodes[i], 'craftedItem_name', tmpName) && tmpName == itemName)
			{
				craftedCount = 1;
				...
				for(k=0; k<ingredients.subNodes.Size(); k+=1)
				{
					...
					if(dm.GetCustomNodeAttributeValueName(ingredients.subNodes[k], 'item_name', tmpName))
					{
						...
						PerfectDismantling_AddOrStackPart(parts, tmpName, tmpInt);
					}
				}
	...
	private final function PerfectDismantling_AddOrStackPart(out parts : array<SItemParts>, itemName : name, quantity : int)
	{
		var part : SItemParts;
		var i : int;
		
		if(!IsNameValid(itemName) || quantity <= 0)
			return;
			
		for(i=0; i<parts.Size(); i+=1)
		{
			if(parts[i].itemName == itemName)
			{
			...
			else if(slot == EES_SkillMutagen1 && player.IsAnyItemEquippedOnSlot(slot))
			{
				...
				else
				{
					...
					else
					{
						...
						} else slot = SSS_GetFirstAvailableMutagenSlot(slot, player); //zur13 modSSS mutTabs
					}
				}
			}
		}
	...
	event OnItemAdded(data : SItemChangedData)
	{
		...
		// ======== modGearLevelScaling 1/1 BEGIN ========
		if (ent == witcher)
			singleItemLevelScaleHandling(itemId);
		// ======== modGearLevelScaling 1/1 END ========
	}
