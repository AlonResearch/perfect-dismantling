...
statemachine class W3PlayerWitcher extends CR4Player
{
	...
	event OnSpawned( spawnData : SEntitySpawnData )
	{
		...
		// modBloodAndSteel ++
		AddAnimEventCallback( 'AllowCustomDodgeBlendLeft', 'OnAnimEvent_AllowCustomDodgeBlendLeft' );
		AddAnimEventCallback( 'AllowCustomDodgeBlendRight', 'OnAnimEvent_AllowCustomDodgeBlendRight' );
		AddAnimEventCallback( 'CustomDodgeRestoreEvade', 'OnAnimEvent_CustomDodgeRestoreEvade' );
		AddAnimEventCallback( 'GeraltFastAttackAnimStart',	'OnAnimEvent_BASGeraltFastAttackAnimStart' );
		AddAnimEventCallback( 'GeraltFastAttackFarAnimStart', 'OnAnimEvent_BASGeraltFastAttackFarAnimStart' );
		AddAnimEventCallback( 'GeraltStrongAttackAnimStart', 'OnAnimEvent_BASGeraltStrongAttackAnimStart' );
		AddAnimEventCallback( 'GeraltStrongAttackFarAnimStart', 'OnAnimEvent_BASGeraltStrongAttackFarAnimStart' );
		// modBloodAndSteel --
	...
	public function ProcessCombatActionBuffer() : bool
	{
		...
		// modBloodAndSteel ++
		if (thePlayer.InCustomDodgeBl())
		{
			return false;
		}
		else if ((action == EBAT_Dodge || action == EBAT_Roll) && thePlayer.InCustomDodgeBlockEvade())
		{
			return false;
		}
		// modBloodAndSteel --
		...
		{			
			...
			case EBAT_SpecialAttack_Light :
			{
				switch ( stage )
				{
					...
					{
						// modBloodAndSteel ++
						if (thePlayer.InCustomDodge())
							thePlayer.SetCombatIdleStance(0.0f);
						// modBloodAndSteel --
	...
	// modBloodAndSteel ++
	event OnAnimEvent_CustomDodgeRestoreEvade( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{
		this.SetInCustomDodgeBl(false);
		this.SetInCustomDodgeBlockEvade(false);
		thePlayer.UnblockAction(EIAB_Counter, 'CustomDodge' );
	...
	event OnAnimEvent_AllowCustomDodgeBlendLeft( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{		
		var dodgeMultId : int;
		
		if ( animEventType == AET_DurationStart && thePlayer.InCustomDodge() && thePlayer.InCustomDodgeBl())
		{
			if (this.BufferCombatAction != EBAT_EMPTY && (this.BufferCombatAction == EBAT_LightAttack || this.BufferCombatAction == EBAT_HeavyAttack) )
			{
				dodgeMultId = this.SetAnimationSpeedMultiplierOverride(1.25f, this.GetDodgeBlSpeedMultId(), true);
				this.SetDodgeBlSpeedMultId(dodgeMultId);
				
				this.SetInCustomDodgeBl(false);
				this.SetInCustomDodgeInvul(false);
				this.SetCombatIdleStance(0.0f);
		...
		{
			this.SetInCustomDodge(false);
			this.SetInCustomDodgeInvul(false);
			this.SetInCustomDodgeBl(false);
			this.SetInCustomDodgeBlockEvade(false);
			this.ResetAnimationSpeedMultiplier( this.GetDodgeBlSpeedMultId() );
		}	
	}
	
	event OnAnimEvent_AllowCustomDodgeBlendRight( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{		
		var dodgeMultId : int;
		...
		if ( animEventType == AET_DurationStart && thePlayer.InCustomDodge() && thePlayer.InCustomDodgeBl())
		{
			if (this.BufferCombatAction != EBAT_EMPTY && (this.BufferCombatAction == EBAT_LightAttack || this.BufferCombatAction == EBAT_HeavyAttack) )
			{
				dodgeMultId = this.SetAnimationSpeedMultiplierOverride(1.27f, this.GetDodgeBlSpeedMultId(), true);
				this.SetDodgeBlSpeedMultId(dodgeMultId);
				
				this.SetInCustomDodgeBl(false);
				this.SetInCustomDodgeInvul(false);
				this.SetCombatIdleStance(1.0f);
	...
	event OnAnimEvent_BASGeraltFastAttackAnimStart( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{
		var animName : name;
		var speed : float;
		...
		switch(animName)
		{
			case 'geralt_fast_6rp_fixed':	speed = 1.3f;	break;
			case 'geralt_fast_5rp_fixed':	speed = 1.35f;	break;
			case 'geralt_fast_4rp_fixed':	speed = 1.68f;	break;			
			case 'geralt_fast_5lp_fixed':	speed = 1.3f;	break;
			default: speed = 1.f;	break;
		}

		SetCustomAnimSpeed(speed);
	}
	
	event OnAnimEvent_BASGeraltStrongAttackAnimStart( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{
		var animName : name;
		var speed : float;
		...
		switch(animName)
		{
			case 'geralt_attack_strong_4_rp_fixed':	speed = 0.99f;	break;
			case 'geralt_attack_strong_5_rp_fixed':	speed = 0.99f;	break;
			
			case 'geralt_attack_strong_3_lp_fixed':	speed = 0.99f;	break;
			case 'geralt_attack_strong_2_lp_fixed':	speed = 1.15f;	break;
			default: speed = 1.05f;	break;
		}
	...
	event OnAnimEvent_BASGeraltFastAttackFarAnimStart( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{
		var animName : name;
		var speed : float;
		...
		switch(animName)
		{
			case 'man_geralt_sword_attack_fast_far_right_2_rp_50ms_mod':	speed = 1.15f;	break;
			case 'man_geralt_sword_attack_fast_far_back_1_rp_50ms_mod':	speed = 1.12f;	break;
			case 'geralt_test_fast_far_back_1_rp_50ms':	speed = 1.13f;	break;
			case 'geralt_test_fast_far_back_2_rp_50ms':	speed = 1.13f;	break;
			case 'geralt_test_fast_far_back_3_rp_50ms':	speed = 1.11f;	break;
			case 'geralt_test_fast_far_back_1_lp_50ms':	speed = 1.09f;	break;
			case 'geralt_test_fast_far_back_2_lp_50ms':	speed = 1.17f;	break;
			case 'man_geralt_sword_attack_fast_far_left_2_rp_50ms_medium':	speed = 1.05f;	break;
			case 'man_geralt_sword_attack_fast_far_left_2_rp_50ms_long':	speed = 0.88f;	break;
			case 'man_geralt_sword_attack_fast_far_left_1_rp_50ms_mod':	speed = 0.97f;	break;
			case 'man_geralt_sword_attack_fast_far_back_1_rp_50ms_mod_fixed':	speed = 0.93f;	break;
			case 'man_geralt_sword_attack_fast_far_left_2_lp_50ms_mod':	speed = 1.1f;	break;
	...
	event OnAnimEvent_BASGeraltStrongAttackFarAnimStart( animEventName : name, animEventType : EAnimationEventType, animInfo : SAnimationEventAnimInfo )
	{
		this.ResetAnimationSpeedMultiplier( this.GetDodgeBlSpeedMultId() );
		SetCustomAnimSpeed(1.1f);
	}
	
	public function SetCustomAnimSpeed( speed : float )
	{
		animID = SetAnimationSpeedMultiplierOverride(speed, animID, true);
	}
	
	private var animID : int;	default animID = -1;
	public function ResetCustomAnimationSpeed()
	{
		ResetAnimationSpeedMultiplier(animID);
	}
	// modBloodAndSteel --
	...
	public function DismantleItem(dismantledItem : SItemUniqueId, toolItem : SItemUniqueId) : bool
	{
		...
		parts = inv.PerfectDismantling_GetDismantlingParts(dismantledItem);
	...
	public function GetMaxRunEncumbrance(out usesHorseBonus : bool) : float
	{
		...
		//AutoLoot +A.S. - increase Geralt's carrying capacity
		//disabled_1: if( StringToInt(theGame.GetInGameConfigWrapper().GetVarValue('AHDAutoLoot_settings', 'invCapacityGeralt')) != 0 )
		//disabled_2 (so the function works even if the mod is disabled from the menu): "&& theGame.GetInGameConfigWrapper().GetVarValue('AHDAutoLoot_settings', 'useAutoLoot')
		value = value + StringToInt(theGame.GetInGameConfigWrapper().GetVarValue('AHDAutoLoot_settings', 'invCapacityGeralt'));
		return value;
	}
	...
	event OnLevelGained(currentLevel : int, show : bool)
	{
		...
		itemLevelScaleHandling(); //modGearLevelScaling 1/1
	}
