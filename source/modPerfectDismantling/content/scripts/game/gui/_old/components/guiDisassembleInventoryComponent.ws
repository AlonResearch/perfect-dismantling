...
class W3GuiDisassembleInventoryComponent extends W3GuiPlayerInventoryComponent
{
	...
	public  function ShouldShowItem( item : SItemUniqueId ):bool
	{
		...
		parts = _inv.PerfectDismantling_GetDismantlingParts(item);
	...
	private function addRecyclingPartsList( item : SItemUniqueId, out flashObject : CScriptedFlashObject ) : void
	{
		...
		for (idx = 0; idx < len; idx+=1)
		{
			...
			curPartData.SetMemberFlashString("name", GetLocStringByKeyExt(_inv.GetItemLocalizedNameByName(curPart.itemName)) + " x" + curPart.quantity);
			curPartData.SetMemberFlashString("txtName", GetLocStringByKeyExt(_inv.GetItemLocalizedNameByName(curPart.itemName)) + " x" + curPart.quantity);
			...
			curPartData.SetMemberFlashInt("reqQuantity", curPart.quantity);
